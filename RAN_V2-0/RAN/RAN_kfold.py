...
class RAN(object):
   ...
   # propagate the activation upward using the weight matrix and activations of the lower layers nodes
    ...
    def calcActivation1(self,weight,activation,index=0,lamda=0.01,layerInfo=None):
        expectedActivation=activation;w=weight;stopCriteria=1;temProj=[];temDevi=[]
        oldDiff=0;ctr=0
        ...
        prevAct=tempAct
        actualActivation=self.propagateUpward(wt,tempAct)
        ...
        while stopCriteria!=2:
            ...
            for delctr in temp1:
                if delctr>0:
                    temptact.append(tempAct[ctr1]+temp1[ctr1]*(1-tempAct[ctr1]))

                else:
                    temptact.append(tempAct[ctr1]+temp1[ctr1]*(tempAct[ctr1]))
                ctr1+=1
