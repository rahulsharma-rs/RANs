...
def test_RAN():
        ...
        # data from .csv file
        path="/home/rahul/RANs-EXperiment/DrugDiscovery/data/drugdataRansForm.csv"
        path = "/home/rahul/Dropbox/MyWork/RAN/NOKATHON/ZQKO_train_RANS_Form.csv"
        #path = "/home/rahul/Dropbox/MyWork/RAN_CRBMv1.0/data/adriano/adriano-data.csv"
        #path="/home/rahul/laurena/realdataRANsform.csv"
        #path = "/home/rahul/laurena/datagpuRANsform.csv"
        #path = "/home/rahul/laurena/datacpuRANsform.csv"
        ...
        trainD, sortedData, label, unique_label, labelFrequency = processData(data=rawData,labeled=True)
        ...
        ran.intermediateData.append(ran.rawData)
        #training
        ran.train_RAN_Upward(ran,isConvex=isConvex,conceptIdentifier={"name":"k-mean","k":2})
        ...
        #--------------------------------------------------------------------------------------------------------
        label1,activation1=unSupervisedLabeling(ran)
        d1 = pandas.DataFrame(label1, columns=['Label'])
        d2 = pandas.DataFrame(ran.intermediateData[1])
        datax = pandas.concat([d2, d1], axis=1)
        fif, ax = plt.subplots()
        ax.set_ylim(-0.2, 1.2)
        ax.set_xlim(-0.2, 1.2)
        parallel_coordinates(datax,'Label',ax=ax)
        ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=12, fancybox=True, shadow=True,prop={'size': 10})
